...
\cite{Aharonian2010} presenta una aproximación para el valor de esta integral. Para ello $x={E_\gamma}/{E_c}$, luego redefinimos la integral anterior como

\begin{equation}\label{eq:mod-28}
    F(x)=x\int_x^\infty K_{5/3}(\tau)d\tau.
\end{equation}

Si tomamos la componente del campo magnético perpendicular a la velocidad del electrón $B_\perp=B\sin(\alpha)$ definimos

\begin{equation}\label{eq:mod-29}
    G(x)=\int\sin(\alpha)F\left(\frac{x}{\sin(\alpha)}\right)\frac{d\Omega}{4\pi}=\frac{1}{2}\int_0^\pi F\left(\frac{x}{\sin(\alpha)}\right)\sin^2(\alpha)d\alpha.
\end{equation}
...
Este comportamiento genera un quiebre en el espectro a bajas frecuencias, que marca la transición entre las regiones ópticamente gruesa y delgada del flujo sincrotrón (ver Figura \ref{fig:synchrotron_break}). 
